...
int main(int argc, char* argv[])
{
	...
	if (rank == 0) {
		...
		char cmd2[] = " /blues/gpfs/home/tshu/project/bebop/MPI_in_MPI/bebop-psm2/Example-Heat_Transfer/heat_transfer_adios2 heat 4 3 40 50 6 5 > output_heat_transfer_adios2.txt 2>&1 &";
		...
		char cmd2[] = " /blues/gpfs/home/tshu/project/bebop/MPI_in_MPI/bebop-psm2/Example-Heat_Transfer/stage_write/stage_write heat.bp staged.bp FLEXPATH \"\" MPI \"\" > output_stage_write.txt 2>&1 &";
		size_t cmdlen = strlen(cmd1) + strlen(machname) + strlen(cmd2) + 1;
		char* mpicmd = (char*) malloc(cmdlen * sizeof(char));
		sprintf(mpicmd, "%s%s%s", cmd1, machname, cmd2);
		system(mpicmd);
