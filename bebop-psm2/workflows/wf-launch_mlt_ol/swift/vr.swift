...
(boolean vld) vr_chk_params(int params[])
{
	int ppn = 36;   // bebop
...
(float exectime) vr_launch_wrapper(string run_id, int params[], int count = 0)
{
	int time_limit = 2;
	if (count < time_limit)
	{
		...
		{
			sleep(1) =>
				exectime = vr_launch_wrapper(run_id, params, count + 1);
		}
		else
		{
			if (exit_code1 != 0)
			...
			{
				exectime = vr_get_exectime(run_id, params);
			}
		}
	}
	else
...
(void v) vr_failure(string run_id, int params[])
{
	string turbine_output = getenv("TURBINE_OUTPUT");
...
(float exectime) vr_get_exectime(string run_id, int params[], int count = 0)
{
	int time_limit = 2;
	if (count < time_limit)
	{
		...
		{
			sleep(1) =>
				exectime = vr_get_exectime(run_id, params, count + 1);
		}
		else
		{
