...
(boolean vld) ht_chk_params(int params[])
{
	int ppn = 36;   // bebop
...
(float exectime) ht_launch_wrapper(string run_id, int params[], int count = 0)
{
	int time_limit = 2;
	if (count < time_limit)
	{
		...
		if (exit_code0 != 0)
		{
			...
			sleep(1) =>
				exectime = ht_launch_wrapper(run_id, params, count + 1);
		}
		else
		{
			int nwork1;
			...
			else
			{
				...
				{
					exectime = ht_get_exectime(run_id, params);
				}
			}
		}
	}
...
(void v) ht_failure(string run_id, int params[])
{
	string turbine_output = getenv("TURBINE_OUTPUT");
	string dir = "%s/run/%s" % (turbine_output, run_id);
...
(float exectime) ht_get_exectime(string run_id, int params[], int count = 0)
{
	int time_limit = 2;
	if (count < time_limit)
	{
		...
		{
			sleep(1) =>
				exectime = ht_get_exectime(run_id, params, count + 1);
		}
		else
		{
