...
(float exectime) hsi_launch_wrapper(string run_id, int params[], int count = 0) 
{
	int time_limit = 2;
	if (count < time_limit)
	{
		...
		if (exit_code0 != 0)
		{
			...
			sleep(1) =>
				exectime = hsi_launch_wrapper(run_id, params, count + 1);
		}
		else
		{
			// Worker counts
			...
			else
			{
				...
				{
					exectime = hsi_get_exectime(run_id, params);
				}
			}
		}
	}
...
(void v) hsi_failure(string run_id, int params[])
{
	string turbine_output = getenv("TURBINE_OUTPUT");
	string dir = "%s/run/%s" % (turbine_output, run_id);
...
(float exectime) hsi_get_exectime(string run_id, int params[], int count = 0)
{
	int time_limit = 2;
	if (count < time_limit)
	{
		...
		{
			sleep(1) =>
				exectime = hsi_get_exectime(run_id, params, count + 1);
		}
		else
		{
