...
emews_root <- Sys.getenv("WORKFLOW_ROOT")
print(paste0("R er: ", emews_root))
if (emews_root == "") {
	r_root <- getwd()
} else {
		...
		)
{
	...
	ctrl = setMBOControlInfill(ctrl, crit = makeMBOInfillCritEI(se.threshold = 1e-6),
