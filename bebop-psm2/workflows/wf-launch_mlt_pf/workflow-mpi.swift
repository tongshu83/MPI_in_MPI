...
(float exectime) launch(string run_id, int params[])
{
	...
	string envs[][];
	envs[0] = [ "OMP_NUM_THREADS="+int2string(params[1]), "swift_chdir="+dir, "swift_output="+dir/"output_hello1.txt", "swift_exectime="+dir/"time_hello1.txt" ];
	envs[1] = [ "OMP_NUM_THREADS="+int2string(params[3]), "swift_chdir="+dir, "swift_output="+dir/"output_hello2.txt", "swift_exectime="+dir/"time_hello2.txt" ];
	...
	if (exit_code == 0) {
		...
		if (time_exit_code == 0) {
			...
			if (exectime >= 0.0) {
				...
				string output = "%0.1i\t%0.1i\t%0.1i\t%0.1i\t%f\t" % (params[0], params[1], params[2], params[3], exectime);
				file out <dir/"time.txt"> = write(output);
			} else {
				printf("swift: The execution time (%f seconds) of the multi-launched application with parameters (%d, %d, %d, %d) is negative.", exectime, params[0], params[1], params[2], params[3]);
			}
		} else {
			exectime = -1.0;
			printf("swift: Failed to get the execution time of the multi-launched application of parameters (%d, %d, %d, %d) with exit code: %d.\n%s", params[0], params[1], params[2], params[3], time_exit_code, time_output);
		}
	} else {
		exectime = -1.0;
		printf("swift: The multi-launched application with parameters (%d, %d, %d, %d) did not succeed with exit code: %d.", params[0], params[1], params[2], params[3], exit_code);
	}
}
		...
		foreach param1 in [params_start[1] : params_stop[1] : params_step[1]]
		{
			foreach param2 in [params_start[2] : params_stop[2] : params_step[2]]
			{
				foreach param3 in [params_start[3] : params_stop[3] : params_step[3]]
				{
					...
					exectime[i] = launch("%0.2i_%0.2i_%0.2i_%0.2i" % (param0, param1, param2, param3), [param0, param1, param2, param3]);
					if (exectime[i] >= 0.0) {
						codes[i] = 0;
					} else {
						codes[i] = 1;
