...
sed -i 's/^dump 1 all custom\/adios_staging .* dump.bp id type x y z vx vy vz fx fy fy fz$/dump 1 all custom\/adios_staging '"$NUM_STEPS"' dump.bp id type x y z vx vy vz fx fy fy fz/' $PWD/experiment/in.quench
sed -i 's/^dump_modify 1 format line "method=.*;have_metadata_file=0"$/dump_modify 1 format line "method='"$METHOD"';have_metadata_file=0"/' $PWD/experiment/in.quench
